...
func (dsp *Display) InitChannels() {
    ...
    dsp.staticImg[0] = NewILIImage(rect)
    dsp.staticImg[1] = NewILIImage(rect)
    dsp.staticImgIdx = 0
...
func (dsp *Display) DrawSync(img image.Image) error {
    ...
    if rect.Empty() {
        //log.Printf("no diffs detected")
        return nil
    }
    //log.Printf("rect enclosing differences: %v", rect)
...
func (dsp *Display) Draw(img image.Image) error {
    ...
    log.Printf("rect enclosing differences: %v", bufItem.rect)
